...
class XMPParseErrorTest {
    ...
    /**
     * A comment inside rdf:RDF is ignored like comments in property element
     * lists, because it carries no data.
     */
    @Test
    fun testCommentInsideRdfIsIgnored() {
        ...
        assertEquals(3, xmpMeta.getPropertyInteger(XMPConst.NS_XMP, "xmp:Rating"))
    }

    /**
     * Input carrying a DOCTYPE is rejected cleanly with an XMPException.
     * XMP never uses DTDs; accepting them would open an entity-expansion
     * attack surface on untrusted input.
     */
    @Test
    fun testDoctypeIsRejected() {

        /* language=XML */
        val testXmp = """
            <!DOCTYPE rdf:RDF [
              <!ENTITY big "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx">
            ]>
            <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
              <rdf:Description rdf:about="&big;"/>
            </rdf:RDF>
        """.trimIndent()
